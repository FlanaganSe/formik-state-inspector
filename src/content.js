    ...
    try {
      ...
      port.onDisconnect.addListener(() => {
        ...
        if (chrome.runtime?.id && !chrome.runtime.lastError) {
          ...
          console.log('[Formik Inspector] Extension context lost, not reconnecting');
        }
      });
    } catch (error) {
      console.warn('[Formik Inspector] Failed to connect to background:', error);
      // Only retry if the error isn't about invalid context
      if (chrome.runtime?.id && !error.message.includes('Extension context invalidated')) {
        setTimeout(connectToBackground, 1000);
      }
    }
  };
